...
A test package that has no dependencies.

NOTE: You must push the `.whl` and the `.tar.gz`.  Otherwise you get weird crashes from pipenv when trying to install.  
...
# publish
pipenv run twine upload --repository codeartifact ./dist/empty_test_package-0.0.1.tar.gz --verbose
```
